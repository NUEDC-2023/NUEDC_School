		...
		if(USE_HORIZONTAL==0)
			{
				...
				LCD_WR_DATA16(Ypos+1);	
				
		  }
			else
				{
					 LCD_WR_REG(lcddev.setxcmd); 
					 LCD_WR_DATA16(Xpos+1);
...
void lcd_showint8(u16 x,u16 y,int dat)
{
    ...
    if(dat<0)
    {
        LCD_ShowChar(x,y*16,'-',16,color2,0);
        dat = -dat;
    }
    else    LCD_ShowChar(x,y*16,' ',16,color2,0);
    ...
    while(i<3)
    {
        LCD_ShowChar(x+(8*(i+1)),y*16,'0' + a[i],16,color2,0);
        i++;
    }
}
...
/*void lcd_showstr(u16 x,u16 y,const int dat[])
{
    ...
    while(dat[j] != '\0')
    {
        LCD_ShowChar(x+8*j,y*16,dat[j],16,BLACK,0);
        j++;
    }
}*/
...
{         	
	...
	{
		num=-num;
		LCD_ShowChar(x-size*len,y,'-',size,color,0);
	}	
	for(t=0;t<len;t++)
	{
...
void lcd_showstr(u16 x,u16 y,u16 width,u8 *p)
{         
	u8 x0=x;
	width=width*8;
	width+=x;
...
//	Draw_Font16B(4,140,RED,"F:shop389957290.taobao.com");
//	
	LCD_Delay_ms(1500);//延时
