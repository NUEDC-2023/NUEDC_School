...
#define LCD_RST_SET   GPIO_SetBits(GPIOB,GPIO_Pin_14)
...
#define LCD_RST_CLR   GPIO_ResetBits(GPIOB,GPIO_Pin_14)

#define	LCD_DC_CLR  	GPIO_ResetBits(GPIOC,GPIO_Pin_5) 
#define	LCD_BLK_CLR  	GPIO_ResetBits(GPIOB,GPIO_Pin_1) 


#define	LCD_SDO   	  GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_14)


#define	LCD_BLK_On          LCD_BLK_SET   		 //开背光  LCD背光--PA8 	    
#define	LCD_BLK_Off         LCD_BLK_CLR    		 //关背光  LCD背光--PA8 	 



#define   Used_HW_SPI     1 //使用单片机硬件SPI接口，则设置为1，使用模拟IO模拟SPI方式，则设置为0



#if    Used_HW_SPI   

#define	LCD_SPI_ReadWriteByte    SPI2_ReadWriteByte    //使用硬件SPI收发数据

#else

#define	LCD_SPI_ReadWriteByte    IO_SPI_ReadWriteByte   //使用 IO模拟 SPI收发数据
...
u8 IO_SPI_ReadWriteByte(u8 TxData);  //使用IO方式模拟SPI读写
...
void LCD_WR_REG(u16 regval);        //液晶屏--写寄存器函数
void LCD_WR_DATA8(u8 data);          //写8位数据
void LCD_WR_DATA16(u16 data);       //写16位数据
...
void lcd_showstr(u16 x,u16 y,u16 width,u8 *p);		//显示一个字符串,12/16字体

void lcd_showint8(u16 x,u16 y,int dat);
void lcd_showint16(u16 x,u16 y,int dat);
...
//void lcd_showstr(u16 x,u16 y,const int dat[]);
